 ...
 * creates a new collection with the same elements as its argument. In effect, 
 * the latter constructor allows the user to copy any collection, producing an
 * equivalent collection of the desired implementation type. There is no way
 * to enforce this convention (as interfaces cannot contain constructors) but
 * all of the general-purpose <code>Collection</code> implementations in the Java
 ...
 * the {@link #addAll(HCollection)} method on an unmodifiable collection may, but is not
 ...
 * to query the presence of an ineligible element may throw an exception, or
 ...
 * insertion of an ineligible element into the collection may throw an
...
public interface HCollection {
	 ...
	 * the specified collection are incompatible with this collection
	 * (optional).
	 * @throws NullPointerException if the specified collection contains one
	 * or more null elements and this collection does not support null elements,
	 ...
	 * 
	 * <p>While the Collection interface adds no stipulations to the general
	 * contract for the Object.equals, programmers who implement the Collection
	 * interface "directly" (in other words, create a class that is a Collection
	 * but is not a Set or a List) must exercise care if they choose to override
	 * the Object.equals. It is not necessary to do so, and the simplest course
	 * of action is to rely on Object's implementation, but the implementor may
	 * wish to implement a "value comparison" in place of the default "reference
	 * comparison." (The List and Set interfaces mandate such value comparisons.)
	 * 
	 * <p>The general contract for the Object.equals method states that equals
	 ...
	 * compared to any list or set. (By the same logic, it is not possible
	 * to write a class that correctly implements both the Set and List
	 * interfaces.)
	 * 
	...
	/**
	 * <p>Returns the hash code value for this collection. While the Collection
	 ...
	 * Object.hashCode method in order to satisfy the general contract
	 * for the Object.hashCodemethod. In particular, c1.equals(c2) implies
	 * that c1.hashCode()==c2.hashCode().
	 * 
	 ...
	 * null. This is useful in determining the length of this collection only
	 * if the caller knows that this collection does not contain any null
	 * elements.)
	 * 
	 ...
	 * 
	 * <p>Suppose l is a List known to contain only strings. The following
	 * code can be used to dump the list into a newly allocated array of
	 * String:
	 * <pre>{@code
	 * String[] x = (String[]) v.toArray(new String[0]);}</pre>
	 * 
