...
public interface HMap {
	...
	/**
	 * A map entry (key-value pair). The <code>Map.entrySet</code> method returns a collection-view of the map,
	 * whose elements are of this class. The <i>only</i> way to obtain a reference to a map entry is from
	 * the iterator of this collection-view. These <code>Map.Entry</code> objects are valid <i>only</i> for the duration
	 ...
	 * own <code>remove</code> operation, or through the <code>setValue</code> operation on a map entry returned by the iterator.
	 * 
	 * 
	 * @author  Daniele Moschetta
	 * @see     HMap
